...
const Footer: React.FC<FooterPropers> = ({ parentHeight }) => {
...

      {
        icon: (
          <svg
            ...
            viewBox="0 0 24 24"
            fill={
              currentPathName == "/community" ? navigatedIconColour : normalIconColour
            }
            className="w-6 h-6 mb-1">
            <path
              fill-rule="evenodd"
        ...
        text: (
          <div
            className={`text-xs font-semibold ${
              currentPathName == "/community" ? "light-white-text" : "navy-black-text"
            }`}>
            Community
          </div>
        ),
        href: homeCouncil.councilValue ? "../community" : "#",
      },
      {
        icon: (
          <svg
  ...
  return (
    <div className="flex flex-col">
      ...
      {usePathname() == "/" && (
        <Slider parentHeight={parentHeight} footerHeight={footerHeight} />
      )}
      <footer
        className={`fixed left-0 bottom-0 creamy-green-bg ${
          usePathname() == "/" ? "" : "rounded-t-lg"
        } w-full z-1002`}
        ref={footerRef}>
